<template>
  <Transition :name="position === 'bottom' ? 'slide' : 'slideinvert'">
    <div
...
import DatePicker from '@VueCtkDateTimePicker/_subs/PickersContainer/_subs/DatePicker/index.vue'
import TimePicker from '@VueCtkDateTimePicker/_subs/PickersContainer/_subs/TimePicker.vue'
import HeaderPicker from '@VueCtkDateTimePicker/_subs/PickersContainer/_subs/HeaderPicker.vue'
import ButtonValidate from '@VueCtkDateTimePicker/_subs/PickersContainer/_subs/ButtonValidate.vue'
...
export default {
  ...
  emits: ['update:model-value', 'close', 'validate'],
  data() {
    return {
      month: this.getMonth(),
  ...
  computed: {
    ...
    height() {
      return !this.onlyTime
        ? this.month
          ? this.month.getMonthDays().length + this.month.getWeekStart() > 35
            ? 347
            : 307
          : 180
        : 200
    ...
    time: {
      ...
      get() {
        return this.modelValue ? moment(this.modelValue, 'YYYY-MM-DD HH:mm').format('HH:mm') : null
      },
    },
    date: {
      set(value) {
  ...
  methods: {
    ...
    getTransitionName(date) {
      const isBigger =
        moment(date) >
        moment(
          `${this.date || moment().format('YYYY-MM-DD')} ${this.time || moment().format('HH:mm')}`,
        )
    ...
    getMonth(payload) {
      if (this.range) {
        ...
        const date =
          rangeVal && (rangeVal.end || rangeVal.start)
            ? moment(rangeVal.end ? rangeVal.end : rangeVal.start)
            : moment()
        return new Month(date.month(), date.year())
      } else if (this.modelValue) {
        return new Month(
          moment(this.modelValue, 'YYYY-MM-DD').month(),
          moment(this.modelValue, 'YYYY-MM-DD').year(),
          this.locale,
        )
      } else {
