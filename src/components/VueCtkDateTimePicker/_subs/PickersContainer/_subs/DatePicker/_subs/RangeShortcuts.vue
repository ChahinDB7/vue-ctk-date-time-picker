...
export default {
      ...
      validator: (val) =>
        val.every((shortcut) => {
          ...
          return (
            shortcut.key &&
            shortcut.label &&
            (isValueInteger || isFunction ? true : SHORTCUT_TYPES.includes(shortcut.value))
          )
        }),
    },
  ...
  methods: {
    init() {
       ...
       */
      if (this.modelValue) {
        const selectedShortcut = this.customShortcuts.find(
          (shortcut) => shortcut.key === this.modelValue,
        )
     ...
     */
    noticeDeprecation() {
      const useDeprecatedAPI = this.customShortcuts.find(
        (shortcut) =>
          typeof shortcut.isSelected !== 'undefined' || typeof shortcut.key === 'undefined',
      )
      if (useDeprecatedAPI)
        console.warn(
          "[vue-ctk-date-time-picker]: You're using a deprecated API. Check the changelog (https://github.com/chronotruck/vue-ctk-date-time-picker/releases) for migration guide.",
        )
    },
    /**
