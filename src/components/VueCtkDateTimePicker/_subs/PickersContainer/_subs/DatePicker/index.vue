  ...
  >
    ...
    <div class="calendar lm-w-100">
      <div class="datepicker-controls flex align-center justify-content-center">
        ...
        <div class="datepicker-container-label flex-1 flex justify-content-center">
          <TransitionGroup
      ...
      <WeekDays :week-days="weekDays" :dark="dark" />
      <div
        :style="{
      ...
      >
        <TransitionGroup :name="transitionDaysName" tag="span">
          <div v-for="m in [month]" :key="m.month" class="datepicker-days flex">
            <div
            ...
            >
              <span v-if="isToday(day)" class="datepicker-today" />
              <span
                v-show="!isDisabled(day) || isSelected(day)"
                :style="bgStyle"
                class="datepicker-day-effect"
              />
              <span v-if="isKeyboardSelected(day)" class="datepicker-day-keyboard-selected" />
...
import RangeShortcuts from '@VueCtkDateTimePicker/_subs/PickersContainer/_subs/DatePicker/_subs/RangeShortcuts.vue'
import YearMonthSelector from '@VueCtkDateTimePicker/_subs/PickersContainer/_subs/DatePicker/_subs/YearMonthSelector.vue'
import WeekDays from '@VueCtkDateTimePicker/_subs/PickersContainer/_subs/DatePicker/_subs/WeekDays.vue'
import CustomButton from '@VueCtkDateTimePicker/_subs/CustomButton/index.vue'
...
export default {
  ...
  emits: ['update:model-value', 'change-month', 'change-year-month'],
  data() {
    return {
  ...
  methods: {
    ...
    isDateEnabled(day) {
      return (
        this.enabledDates.length === 0 || this.enabledDates.indexOf(day.format('YYYY-MM-DD')) > -1
      )
    },
    isBeforeMinDate(day) {
    ...
    isSelected(day) {
      const date = [
        ...(this.modelValue && this.modelValue.start
          ...
          : this.range
            ? []
            : [moment(this.modelValue).format('YYYY-MM-DD')]),
    ...
    firstInRange(day) {
      return this.modelValue && this.modelValue.start
        ? moment(moment(this.modelValue.start).format('YYYY-MM-DD')).isSame(
            day.format('YYYY-MM-DD'),
          )
        : false
    },
    lastInRange(day) {
      return this.modelValue && this.modelValue.end
        ? moment(moment(this.modelValue.end).format('YYYY-MM-DD')).isSame(day.format('YYYY-MM-DD'))
        : false
    },
    ...
    selectDate(day) {
      ...
      if (this.range) {
        ...
        if (!modelValue.start || modelValue.end || day.isBefore(moment(modelValue.start))) {
          modelValue.start = day.format('YYYY-MM-DD')
          modelValue.end = null
        } else {
          modelValue.end = day.format('YYYY-MM-DD')
        }
    ...
    selectYearMonth(event) {
      ...
      const isBefore = year === this.month.year ? month < this.month.month : year < this.month.year
