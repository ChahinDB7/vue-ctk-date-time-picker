<template>
  <div :style="bgStyle" class="header-picker" :class="{ 'is-dark': dark }">
    <div v-if="!onlyTime" class="header-picker-year">
      <TransitionGroup :name="transitionName" tag="span">
        <div v-for="y in [year]" :key="y">
          {{ y }}
        </div>
      </TransitionGroup>
    </div>

    <div v-if="!range" class="flex justify-content-between">
      <TransitionGroup
        v-if="!onlyTime"
        :name="transitionName"
        tag="span"
      ...
      >
        <span v-for="dateFormatted in [getDateFormatted]" :key="dateFormatted">
      ...
      >
        ...
        >
          <span v-for="hour in [dateTime.format('HH')]" :key="hour">
            {{ hour }}
          </span>
        </TransitionGroup>
        <span>:</span>
        ...
        >
          <span v-for="min in [dateTime.format('mm')]" :key="min">
            {{ min }}
          </span>
        </TransitionGroup>
      </div>
...
export default {
  ...
  computed: {
    ...
    isFormatTwelve() {
      return this.format
        ? this.format.indexOf("a") > -1 || this.format.indexOf("A") > -1
        : false;
    },
    getRangeDatesFormatted() {
      ...
      if (!hasStartValues && !hasEndValues) {
        ...
        const datesFormatted = hasStartValues
          ? `${moment(this.modelValue.start).format("ll")}`
          : "...";
        return hasEndValues
          ? `${datesFormatted} - ${moment(this.modelValue.end).format("ll")}`
          : `${datesFormatted} - ...`;
      } else {
        return null;
      }
