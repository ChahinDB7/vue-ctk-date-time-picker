<template>
  ...
  >
    ...
    <div v-if="hasPickerOpen && overlay" class="time-picker-overlay" @click.stop="closePicker" />
...
import CustomInput from '@VueCtkDateTimePicker/_subs/CustomInput/index.vue'
import PickersContainer from '@VueCtkDateTimePicker/_subs/PickersContainer/index.vue'
...
const updateMomentLocale = (locale, firstDayOfWeek) => {
  ...
  if (firstDayOfWeek) {
    const firstDayNumber =
      Number.isInteger(firstDayOfWeek) && firstDayOfWeek === 0
        ? 7
...
export default {
  ...
  inheritAttrs: false,
  props,
  emits: ['update:model-value', 'formatted-value', 'destroy', 'is-hidden', 'is-shown', 'validate'],
  data() {
    return {
      pickerOpen: false,
          ...
          ? {
              start:
                this.modelValue && this.modelValue.start
                  ? moment(this.modelValue.start, this.formatOutput).format('YYYY-MM-DD')
                  : null,
              end:
                this.modelValue && this.modelValue.end
                  ? moment(this.modelValue.end, this.formatOutput).format('YYYY-MM-DD')
                  : null,
            }
          : this.getDateTime()
        return dateTime
  ...
  methods: {
    getRangeDatesFormatted() {
      ...
      if (hasStartValues || hasEndValues) {
        const datesFormatted = hasStartValues
          ? `${moment(this.modelValue.start, this.formatOutput).set({ hour: 0, minute: 0, second: 0 }).format(this.formatted)}`
          : '...'
        return hasEndValues
          ? `${datesFormatted} - ${moment(this.modelValue.end, this.formatOutput).set({ hour: 23, minute: 59, second: 59 }).format(this.formatted)}`
          : `${datesFormatted} - ...`
      } else {
        return null
      }
    ...
    getDateTimeToSend(value) {
      ...
      const dateToSend = dateTime ? moment(dateTime, 'YYYY-MM-DD HH:mm') : null
      const dateTimeToSend = dateToSend
        ? nearestMinutes(this.minuteInterval, moment(dateToSend), 'YYYY-MM-DD HH:mm').format(
            this.formatOutput,
          )
        : null
    ...
    getDateTime() {
      const date = this.modelValue ? moment(this.modelValue, this.formatOutput) : null
      return date
        ? nearestMinutes(this.minuteInterval, date, this.formatOutput).format('YYYY-MM-DD HH:mm')
        : null
    },
    /**
